...
class CsvMonitor extends CI_Controller
{
    ...
    */
        ...
        $assetTypeFilter = trim((string) $this->input->get('asset_type'));
        $catTypeFilter = trim((string) $this->input->get('cat_type'));
    ...
    */
        ...
        $sortBy = trim((string) $this->input->get('sort_by'));
        ...
        $sortOrder = strtolower(
            trim((string) $this->input->get('sort_order'))
        );
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | This makes the viewer tolerant of header capitalization
    ...
    */
        ...
        foreach ($header as $index => $columnName) {
            ...
            $columnIndex[$normalizedName] = $index;
        }
    ...
    */
        ...
        $barcodeIndex = isset($columnIndex['barcode'])
            ? $columnIndex['barcode']
            : null;

        $descriptionIndex = isset($columnIndex['desc'])
            ? $columnIndex['desc']
            : null;

        $remarksIndex = isset($columnIndex['remarks'])
            ? $columnIndex['remarks']
            : null;

        $assetTypeIndex = isset($columnIndex['asset_type'])
            ? $columnIndex['asset_type']
            : null;

        $catTypeIndex = isset($columnIndex['cat_type'])
            ? $columnIndex['cat_type']
            : null;

        $imageIndex = isset($columnIndex['image'])
            ? $columnIndex['image']
            : null;

        $datetimeIndex = isset($columnIndex['datetime_saved'])
            ? $columnIndex['datetime_saved']
            : null;
    ...
    |--------------------------------------------------------------------------
    |
    | Build dropdown values while reading the CSV.
    |
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | If no sort is selected, we only keep 50 rows.
    |
    | If sorting is requested, all matching rows are temporarily
    | collected so they can be sorted before pagination.
    |
    */
...
*/
            ...
            foreach ($columnIndex as $columnName => $columnPosition) {

                $assocRow[$columnName] =
                    isset($row[$columnPosition])
                    ? $row[$columnPosition]
                    : '';
            }
        ...
        */
            ...
            $barcode = (
                $barcodeIndex !== null &&
                isset($row[$barcodeIndex])
            )
                ? trim((string) $row[$barcodeIndex])
                : '';
            ...
            $remarks = (
                $remarksIndex !== null &&
                isset($row[$remarksIndex])
            )
                ? trim((string) $row[$remarksIndex])
                : '';

            $assetType = (
                $assetTypeIndex !== null &&
                isset($row[$assetTypeIndex])
            )
                ? trim((string) $row[$assetTypeIndex])
                : '';

            $catType = (
                $catTypeIndex !== null &&
                isset($row[$catTypeIndex])
            )
                ? trim((string) $row[$catTypeIndex])
                : '';
        ...
        */
            ...
            if ($sortBy !== '') {
                ...
                $matchingRows[] = array(
                    ...
                    'image' => (
                        $imageIndex !== null &&
                        isset($row[$imageIndex])
                    )
                        ? trim((string) $row[$imageIndex])
                        : '',
                    'datetime_saved' => (
                        $datetimeIndex !== null &&
                        isset($row[$datetimeIndex])
                    )
                        ? trim((string) $row[$datetimeIndex])
                        : ''
                );
            } else {
            ...
            -----------------------------------------------------------
            |
            | Only keep the 50 rows needed for the current page.
            |
            */

                if (
                    $totalCount > $offset &&
                    count($pageRows) < $perPage
                ) {
                    $pageRows[] = $assocRow;
                }
            }
        ...
        if (
            ...
            usort(
                ...
                function ($a, $b) use ($sortBy, $sortOrder) {
                    ...
                    $valueA = isset($a[$sortBy])
                        ? $a[$sortBy]
                        : '';

                    $valueB = isset($b[$sortBy])
                        ? $b[$sortBy]
                        : '';
    ...
    */
        ...
        $catTypes = array_keys($catTypes);

        natcasesort($assetTypes);
        natcasesort($catTypes);

        $assetTypes = array_values($assetTypes);
        $catTypes = array_values($catTypes);
    ...
    */
        ...
        $data = array(
            ...
            // Only the current 50 rows are sent to the browser.
            'rows' => $pageRows,
