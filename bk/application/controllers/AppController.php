...
class AppController extends CI_Controller {
	...
	public function uploadNfCsv()
	{
		...
		if ($header !== false) {
			...
			if ($descIdx === false || $remarksIdx === false || $datetimeIdx === false) {
				flock($fp, LOCK_UN);
				fclose($fp);
				echo json_encode([
					'status' => 'error',
			...
			while (($row = fgetcsv($fp)) !== false) {
				...
				$remarks = trim($row[$remarksIdx]);
				$key = strtolower($desc . '|' . $remarks);
		...
		foreach ($uploadedMap as $key => $row) {
			if (isset($existingMap[$key])) {
				...
				$existingDatetime = $existingMap[$key][$datetimeIdx];
				$uploadedDatetime = $row[$datetimeIdx];
				...
				if (!$existingFullySaved && $uploadedFullySaved) {
					$existingMap[$key][$datetimeIdx] = $uploadedDatetime;
				}
		...
		if (isset($_FILES['images']['name']) && is_array($_FILES['images']['name'])) {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				$destination = $imageUploadPath . $imageFilename;
	...
	// public function getNfCsvFiles()
	// {
	// 	$csvPath = FCPATH . 'nfitems/';
	...
	// 							'url' => base_url(
	// 								'nfitems_images/' . rawurlencode(trim($row[$imageIdx]))
	// 							)
	// 						];
	// 					}
	// 				}
